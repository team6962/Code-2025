 ...
 *
 * The pivot can be controlled with the {@link #deploy()} method and {@link #stow()} method, which
 * return {@link Command Commands}.
 *
 * <h3>Getting the Pivot's State</h3>
 *
 * The position and velocity of the pivot can be read with the {@link #getPosition()} and {@link
 * #getVelocity()} methods.
 *
 * <h3>Automatic Holding Behavior</h3>
 *
 * When no command is actively controlling the pivot with an ongoing {@link #moveTo()} or {@link
 * #holdAt()} command, the pivot will use feedback control to stay in its current position.
 *
 * <h3>Simulation</h3>
 *
 * In simulation, instantiate an instance of the {@link IntakePivotSim} subclass instead of this
 * one. The IntakePivotSim class fully simulates the pivot's physics and motor behavior.
 *
 * <h3>Internal Position Offset</h3>
 *
 * The TalonFX's position is offset by the angle between the center of mass and the external code's
 * zero angle. This means that if the center of mass is 10 degrees above the zero angle, then when
 * the external code sets the pivot to 0 degrees, the TalonFX will actually be set to 10 degrees.
 * This offset is handled internally and does not affect the external code's view of the pivot's
 * position.
 */
public class IntakePivot extends SubsystemBase {
  ...
  public IntakePivot(IntakeSensors sensors) {
    ...
    encoderConfig
        ...
        .withMagnetOffset(
            IntakeConstants.absoluteEncoderOffset.times(IntakeConstants.pivotSensorToMechanism))
        .withAbsoluteSensorDiscontinuityPoint(
            IntakeConstants.absoluteEncoderDiscontinuityPoint.times(
    ...
    MechanismLigament2d ligament =
        MechanismLogger.getLigament("Intake Pivot", 14.4, getCenterOfMassPosition().in(Degrees));
  ...
  /**
   * Gets the position the pivot should move to when a command ends, based on the previous target
   * position.
   *
  ...
  /**
   * Creates a command that moves the pivot to the specified angle, ending when the target is
   * reached.
   *
  ...
  /**
   * Creates a command that moves the pivot to the intake position, ending when the target is
   * reached.
   *
  ...
  /**
   * Creates a command that moves the pivot to the stow position, ending when the target is reached.
   *
  ...
  /**
   * Returns whether the pivot is near the specified position, within the tolerance specified in
   * {@link IntakeConstants#pivotTolerance}.
   *
  ...
  private Current calculateGravityFeedforwardCurrent() {
    return IntakeConstants.kG.times(
        Math.cos(getPosition().plus(IntakeConstants.centerOfMassAngularOffset).in(Radians)));
  }
  ...
  private void initStatusSignals() {
    ...
    CTREUtils.check(
        BaseStatusSignal.setUpdateFrequencyForAll(
            ...
            velocityIn,
            encoder.getPosition(),
            encoder.getVelocity() // TODO: Test to see if this is needed
            ));

    CTREUtils.check(
        BaseStatusSignal.setUpdateFrequencyForAll(
            Hertz.of(50), statorCurrentIn, supplyCurrentIn, appliedVoltageIn));
  ...
  private void refreshStatusSignals() {
    ...
    CTREUtils.check(
        BaseStatusSignal.refreshAll(
            positionIn, velocityIn, statorCurrentIn, supplyCurrentIn, appliedVoltageIn));
  }
