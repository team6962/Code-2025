...
public class TunableNumber extends SubsystemBase {
  ...
  public TunableNumber(
      SubsystemBase subsystem, String name, Consumer<Double> setter, double defaultNumber) {
    this.name = subsystem.getClass().getSimpleName() + "/" + name;
    this.setter = setter;
    tab = Shuffleboard.getTab("Tunable Numbers");
    currentNumber = defaultNumber;
    entry = tab.add(name, currentNumber).withWidget(BuiltInWidgets.kNumberSlider).getEntry();
  }
