...
public class PositionDeltaMovement implements SwerveMovement {
    ...
    public PositionDeltaMovement(
        ...
        if (initialPositions.length != 4 || positionDeltas.length != 4 || relativeVelocities.length != 4) {
            throw new IllegalArgumentException("All input arrays must have a length of 4.");
        }
    }

    private SwerveModulePosition[] getOptimizedPositionTargets(SwerveModulePosition[] currentPositions) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            double optimizationAdjustmentRotations = MathUtils.floorMod(unoptimizedTargetAngleRotations - currentAngleRotations - 0.5, 1) - 0.5;
            ...
            SwerveModulePosition optimizedTarget = new SwerveModulePosition(
    ...
    public void execute(SwerveCore drivetrain) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            module.drive(
                ...
                SwerveMovement.motionMagicExpoVoltage
                    .withPosition(steerAngle)
            );
        }
    }
