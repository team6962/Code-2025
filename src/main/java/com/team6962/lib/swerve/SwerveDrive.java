                ...
                new Command() {
                  ...
                  public void end(boolean interrupted) {
                    Angle gyroAngleChange =
                        Rotations.of(
                            getContinuousGyroscopeAngle().minus(initialGyroAngle).in(Rotations));
                    Distance driveRadius = getConstants().chassis().driveRadius();

                    Logger.log(
                        "Swerve Drive/Wheel Size Calibration/Gyro Angle Change", gyroAngleChange);
                    ...
                    for (int i = 0; i < 4; i++) {
                      wheelDiameters[i] =
                          driveRadius.times(gyroAngleChange.div(wheelAngleChanges[i])).times(2);
                    ...
                    Logger.log(
                        "Swerve Drive/Wheel Size Calibration/Average Diameter", averageDiameter);
                  }
                },
            Set.of())
        .alongWith(
