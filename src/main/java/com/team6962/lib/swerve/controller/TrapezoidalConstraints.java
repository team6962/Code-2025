...
public class TrapezoidalConstraints {
  ...
  public static double computeTrapezoidalProfileTime(
      TrapezoidalConstraints constraints, double distance) {
    double minimumTrapezoidalProfileDistance =
    ...
    if (distance >= minimumTrapezoidalProfileDistance) {
      return constraints.maxVelocity / constraints.maxAcceleration
          + distance / constraints.maxVelocity;
    } else {
      return 2 * Math.sqrt(distance / constraints.maxAcceleration);
    }
  }

  public static record TrapezoidProfileAndDistance(
      TrapezoidalConstraints constraints, double distance) {
    @Override
  ...
  public static TrapezoidalConstraints scaleTrapezoidalConstraintsToTime(
      TrapezoidalConstraints constraints, double time, double distance) {
    double trapezoidalMaxVelocity =
        distance / (time - constraints.maxVelocity / constraints.maxAcceleration);
    double trapezoidalMaxAcceleration =
        constraints.maxAcceleration / constraints.maxVelocity * trapezoidalMaxVelocity;
    ...
    double triangularMaxVelocity =
        constraints.maxVelocity / constraints.maxAcceleration * triangularMaxAcceleration;
  ...
  public static TrapezoidalConstraints[] matchProfileDurations(
      TrapezoidProfileAndDistance... profiles) {
    ...
    for (int i = 0; i < profiles.length; i++) {
      ...
      matchedProfiles[i] =
          scaleTrapezoidalConstraintsToTime(
              profiles[i].constraints, targetTime, profiles[i].distance);
    }
