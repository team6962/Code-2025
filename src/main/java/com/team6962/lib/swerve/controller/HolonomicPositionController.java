...
public class HolonomicPositionController implements AutoCloseable {
  ...
  public HolonomicPositionController(
    ...
    this.xController =
        new PositionController(linearProfile, linearFeedback)
            .withLogging("HolonomicPositionController/XMeters");
    this.yController =
        new PositionController(linearProfile, linearFeedback)
            .withLogging("HolonomicPositionController/YMeters");
    this.thetaController =
        new PositionController(angularProfile, angularFeedback)
            .withLogging("HolonomicPositionController/ThetaRadians");
  }
  ...
  public static class State {
    ...
    public boolean equals(State other) {
      return this.position.equals(other.position)
          ...
          && Math.abs(this.velocity.omegaRadiansPerSecond - other.velocity.omegaRadiansPerSecond)
              < 1e-6;
    }
  }
  ...
  public ChassisSpeeds calculate(double time, State start, State end, State current) {
    Angle startAngle =
        MeasureMath.minDifference(
            start.position.getRotation().getMeasure(), end.position.getRotation().getMeasure());
    Angle currentAngle =
        MeasureMath.minDifference(
            current.position.getRotation().getMeasure(), end.position.getRotation().getMeasure());
    Angle endAngle = Radians.of(0);

    TrapezoidProfile.State optimizedAngularEndState =
        new TrapezoidProfile.State(endAngle.in(Radians), end.velocity.omegaRadiansPerSecond);

    Vector2 pathDirection =
        LineFollowing.directionBetweenPoints(
            new Vector2(start.position), new Vector2(end.position));
    Vector2 startPosition = new Vector2(0, 0);
    Vector2 endPosition =
        new Vector2(
            end.position.getTranslation().minus(start.position.getTranslation()).getNorm(), 0);
    Vector2 startVelocity =
        LineFollowing.fieldToPathDisplacement(new Vector2(start.velocity), pathDirection);
    Vector2 endVelocity =
        LineFollowing.fieldToPathDisplacement(new Vector2(end.velocity), pathDirection);
    Vector2 currentVelocity =
        LineFollowing.fieldToPathDisplacement(new Vector2(current.velocity), pathDirection);
    Vector2 currentPosition =
        LineFollowing.fieldToPathPosition(
            new Vector2(current.position), new Vector2(start.position), pathDirection);

    Vector2 velocity =
        LineFollowing.pathToFieldDisplacement(
            new Vector2(
    ...
    return new ChassisSpeeds(
        ...
        thetaController.calculateVelocity(
            ...
            new TrapezoidProfile.State(
                startAngle.in(Radians), start.velocity.omegaRadiansPerSecond),
            optimizedAngularEndState,
            new TrapezoidProfile.State(
                currentAngle.in(Radians), current.velocity.omegaRadiansPerSecond)));
  }

  public boolean isFinished() {
    return xController.isFinished() && thetaController.isFinished();
  }
