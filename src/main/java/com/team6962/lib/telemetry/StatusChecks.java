...
/**
 * {@code StatusChecks} is a utility class that provides a way to add boolean status
 * checks to Shuffleboard. It is intended to be used to monitor the health of
 * various subsystems and components before and during a match.
 * <p>You can add a check by calling the {@code add} method on a {@code Category}
 * object, gotten by calling the {@code under} method on a {@code StatusChecks}
 * instance:</p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <p>You can also add checks for TalonFX controllers, CANcoders, and Spark MAX
 * controllers:</p>
 * 
 * <pre>
...
public final class StatusChecks {
    ...
    public static class Category {
        ...
        public void add(String name, BooleanSupplier checkSupplier) {
            GenericEntry entry = grid.add(name, checkSupplier.getAsBoolean())
        ...
        public void add(String name, TalonFX motor) {
            ...
            add(motor.getDeviceID() + ". " + name + " Faults", () -> CTREUtils.unwrap(motor.getFaultField()) != 0 || CTREUtils.unwrap(motor.getStickyFaultField()) != 0);
        }

        public void add(String name, CANcoder encoder) {
            add(encoder.getDeviceID() + ". " + name + " Alive", () -> encoder.getVersion().getStatus().isOK());
            add(encoder.getDeviceID() + ". " + name + " Faults", () -> CTREUtils.unwrap(encoder.getFaultField()) != 0 || CTREUtils.unwrap(encoder.getStickyFaultField()) != 0);
        }

        public void add(String name, SparkMax motor) {
            add(motor.getDeviceId() + ". " + name + " Alive", () -> motor.getFirmwareVersion() != 0);
            add(motor.getDeviceId() + ". " + name + " Faults", () -> motor.getFaults().rawBits != 0 || motor.getStickyFaults().rawBits != 0);
        }
    }
}
