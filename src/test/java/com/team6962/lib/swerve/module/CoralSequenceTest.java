...
public class CoralSequenceTest {
  ...
  public void simpleIntersectTest() {
    Distance length =
        CoralSequences.estimatePathTranslationLength(
    ...
    Distance expectedLength =
        perfectLength
            .minus(CoralSequences.ESTIMATED_REEF_AVOIDANCE_DIAMETER)
            .plus(reefRadius.times(Math.PI));
  ...
  public void diagonalIntersectionTest() {
    ...
    Distance length =
        CoralSequences.estimatePathTranslationLength(
            ReefPositioning.REEF_CENTER.plus(start), ReefPositioning.REEF_CENTER.plus(end));
